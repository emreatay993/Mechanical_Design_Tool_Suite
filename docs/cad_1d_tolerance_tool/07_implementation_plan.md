...
- Add an OCCT AIS/V3d primary CAD viewer adapter embedded in PyQt6.
...
### T04A Primary CAD Viewer Spike

Goal: Prove the final-product CAD viewer direction using OCCT AIS/V3d embedded in PyQt6, without making tessellated mesh data authoritative.

Preconditions: T04 geometry adapter can import STEP/IGES and expose live OCCT shapes through `OccCadGeometrySession.kernel_shape()`. Use `environment-cad312.yml` or an equivalent Python 3.12 environment with `pythonocc-core` `novtk` and PyQt6.

Conservative write scope: new `cad_viewer_api.py`, `cad_viewer_occ.py`, viewer-specific tests or smoke harnesses, and docs.

Deliverables: PyQt6 viewer host using `load_backend("pyqt6")` and `qtViewer3d`, display of imported fixture shapes from the geometry session, fit/orbit/pan/zoom basics, nonblank render verification, and explicit mapping plan from AIS selections back to `ShapeReference` ids.

Verification: `$env:PYTHONNOUSERSITE="1"; $env:PYTHONPATH="src"; & "C:\ProgramData\miniforge3\envs\mdts-cad312\python.exe" -s -m unittest discover -s tests -p "test_cad_viewer*.py"` plus a manual smoke run that displays `neutral_step_two_part_loop.step`.

Non-goals: Full guided stackup workflow, production annotation editing, mesh-only viewer fallback, native commercial CAD import.

Packetization notes: This should run after P03/T04 and before full P04/P05 UI workflow work. If pythonocc's PyQt6 viewer helper is unstable, preserve the viewer API and plan a C++ Qt6 + OCCT viewport adapter rather than switching the primary viewer to PyVista.
...
Conservative write scope: `cad_tolerance_gui.py`, `cad_tolerance_viewmodels.py`, `cad_viewer_api.py`, `cad_viewer_occ.py`, Qt resources/icons if needed, GUI tests.

Deliverables: Main window, ribbon-like tabs/actions, left assembly tree, center OCCT AIS/V3d viewport host or explicitly isolated viewer stub, right summary/detail pane, table models with required columns, observed row colors, dashboard badges, non-1D warning treatment, and documented fidelity gaps for unreadable labels.
...
| T04A | P04A Primary CAD viewer spike | gpt-5.5 xhigh |
...
- Run primary viewer tests in `mdts-cad312` with `PYTHONNOUSERSITE=1` so PyQt6 and the `novtk` OCCT runtime are validated without user-site package leakage.
...
- OCCT AIS/V3d is the primary CAD viewer path; PyVista/VTK is secondary diagnostics only, not the authoritative CAD viewer.
